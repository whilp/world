...
local function test_get_check_runs()
  ...
  local runs = check_logs.get_check_runs("owner", "repo", "abc123", "token", {fetch = mock_fetch})
...
local function test_get_pr_comments()
  local mock_fetch = function(url, opts)
    ...
    return 200, {}, cosmo.EncodeJson({
      ...
      {id = 2, body = "<!-- check-logs -->\n# Logs\nfoo"},
    })
  end

  local comments = check_logs.get_pr_comments("owner", "repo", 42, "token", {fetch = mock_fetch})
...
local function test_find_cosmic_comment()
  ...
  local found = check_logs.find_cosmic_comment(comments)
...
local function test_find_cosmic_comment_not_found()
  ...
  local found = check_logs.find_cosmic_comment(comments)
  assert(not found, "expected not to find check logs comment")
end
...
local function test_create_pr_comment()
  ...
  local ok = check_logs.create_pr_comment("owner", "repo", 42, "test body", "token", {fetch = mock_fetch})
...
local function test_update_pr_comment()
  ...
  local ok = check_logs.update_pr_comment("owner", "repo", 123, "updated body", "token", {fetch = mock_fetch})
...
local function test_create_comment_body()
  local body = check_logs.create_comment_body("test logs")
  assert(body:find("<!-- check-logs -->", 1, true), "expected marker")
...
local function test_extract_logs_from_comment()
  ...
  local logs = check_logs.extract_logs_from_comment(comment_body)
  assert(logs, "expected logs to be extracted")
  assert(logs:find("check-logs", 1, true), "expected marker in logs")
