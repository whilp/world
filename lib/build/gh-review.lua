...
local function fetch_reviews(info)
  local url = string.format("https://api.github.com/repos/%s/%s/pulls/%s/reviews",
    info.owner, info.repo, info.pr)

  return fetch_json(url)
end

local function fetch_review_comments(info, review_id)
  local url = string.format("https://api.github.com/repos/%s/%s/pulls/%s/reviews/%s/comments",
    info.owner, info.repo, info.pr, review_id)
...
local function print_review_comments(info, opts)
  ...
  local reviews, err = fetch_reviews(info)
  if not reviews then
    return nil, err
  end

  if info.review then
    local comments, err = fetch_review_comments(info, info.review)
