...
local function write_result_file(filepath, status, message, stdout, stderr)
...
local function test_single_checker_mode()
  ...
  assert(stdout:match("2 tests: 1 passed, 1 failed"), "expected test summary")
...
local function test_update_mode_summary()
  ...
  assert(stdout:match("2 checked, 1 updates available"), "expected update summary format")
...
local function test_skip_and_ignore()
  ...
  assert(code == 0, "expected exit code 0")
  assert(stdout:match("3 tests: 1 passed, 0 failed, 1 skipped, 1 ignored"), "expected correct counts")
...
local function test_missing_dir_flag()
  local code, stdout, stderr = run_reporter("test", "file.test.ok")
...
local function test_no_files()
  local code, stdout, stderr = run_reporter("--dir", tmpdir, "test")

  assert(code == 1, "expected exit code 1")
  assert(stderr:match("no files specified"), "expected error about no files")
end
test_no_files()
