...
```lua
unix.EADDRINUSE (constant)
```

Address already in use. Raised by `bind`, `connect`, `listen`.

### EADDRNOTAVAIL

```lua
unix.EADDRNOTAVAIL (constant)
```

Address not available. Raised by `bind`, `connect`.

### EAFNOSUPPORT

```lua
unix.EAFNOSUPPORT (constant)
```

Address family not supported. Raised by `connect`, `socket`, `socketpair`.

### EAGAIN

```lua
unix.EAGAIN (constant)
```
...
```lua
unix.EALREADY (constant)
```

Connection already in progress. Raised by `connect`, `send`.
...
```lua
unix.ECONNABORTED (constant)
```

Connection reset before accept. Raised by `accept`.

### ECONNREFUSED

```lua
unix.ECONNREFUSED (constant)
```
...
```lua
unix.ECONNRESET (constant)
```

Connection reset by client. Raised by `send`.

### EDEADLK

```lua
unix.EDEADLK (constant)
```
...
```lua
unix.EDESTADDRREQ (constant)
```

Destination address required. Raised by `send`, `write`.

### EDQUOT

```lua
unix.EDQUOT (constant)
```
...
```lua
unix.EHOSTDOWN (constant)
```

Host is down. Raised by `accept`.

### EHOSTUNREACH

```lua
unix.EHOSTUNREACH (constant)
```

Host is unreachable. Raised by `accept`.
...
Identifier removed. Raised by `msgctl`.
...
The greatest of all errnos; crucial for building real time reliable software.
...
```lua
unix.EISCONN (constant)
```

Socket is connected. Raised by `connect`, `send`.

### EISDIR

```lua
unix.EISDIR (constant)
```
...
```lua
unix.EMSGSIZE (constant)
```

Message too long. Raised by `send`.

### ENAMETOOLONG

```lua
unix.ENAMETOOLONG (constant)
```
...
```lua
unix.ENETDOWN (constant)
```

Network is down. Raised by `accept`.

### ENETRESET

```lua
unix.ENETRESET (constant)
```
...
```lua
unix.ENETUNREACH (constant)
```

Host is unreachable. Raised by `accept`, `connect`.

### ENFILE

```lua
unix.ENFILE (constant)
```
...
```lua
unix.ENOEXEC (constant)
```

Exec format error. Raised by `execve`, `uselib`.

### ENOLCK

```lua
unix.ENOLCK (constant)
```

No locks available. Raised by `fcntl`, `flock`.

### ENOMSG

```lua
unix.ENOMSG (constant)
```
...
```lua
unix.ENOPROTOOPT (constant)
```

Protocol not available. Raised by `getsockopt`, `accept`.

### ENOSPC

```lua
unix.ENOSPC (constant)
```
...
```lua
unix.ENOSYS (constant)
```

System call not available on this platform. On
...
```lua
unix.ENOTBLK (constant)
```

Block device required. Raised by `umount`.

### ENOTCONN

```lua
unix.ENOTCONN (constant)
```
...
```lua
unix.ENOTDIR (constant)
```

Not a directory. This means that a directory
...
```lua
unix.ENOTTY (constant)
```

Inappropriate i/o control operation. Raised by `ioctl`.
...
No such device or address. Raised by `lseek`, `open`, `prctl`.

### EOPNOTSUPP

```lua
unix.EOPNOTSUPP (constant)
```
...
```lua
unix.EPROTONOSUPPORT (constant)
```

Protocol not supported. Raised by `socket`, `socketpair`.

### EPROTOTYPE

```lua
unix.EPROTOTYPE (constant)
```

Protocol wrong type for socket. Raised by `connect`.

### ERANGE

```lua
unix.ERANGE (constant)
```
...
```lua
unix.ESHUTDOWN (constant)
```

Cannot send after transport endpoint shutdown; note that shutdown write is an `EPIPE`.

### ESOCKTNOSUPPORT

```lua
unix.ESOCKTNOSUPPORT (constant)
```
...
Timer expired. Raised by `connect`.

### ETIMEDOUT

```lua
unix.ETIMEDOUT (constant)
```

Connection timed out. Raised by `connect`.

### ETOOMANYREFS

```lua
unix.ETOOMANYREFS (constant)
```

Too many references: cannot splice. Raised by `sendmsg`.

### ETXTBSY

```lua
unix.ETXTBSY (constant)
```
...
```lua
unix.O_SYNC (constant)
```

synchronize i/o operations appropriately

### O_TMPFILE

```lua
unix.O_TMPFILE (constant)
```

try to make temp more secure (Linux and Windows only)

### O_TRUNC

```lua
unix.O_TRUNC (constant)
```
...
```lua
unix.O_UNLINK (constant)
```

automatically delete file upon close()

### O_WRONLY

```lua
unix.O_WRONLY (constant)
```
...
```lua
unix.PLEDGE_PENALTY_KILL_PROCESS (constant)
```

Causes the process and all its threads to be killed.
This is always the case on OpenBSD.

### PLEDGE_PENALTY_KILL_THREAD

```lua
unix.PLEDGE_PENALTY_KILL_THREAD (constant)
```

Causes the violating thread to be killed. This is
...
```lua
unix.PLEDGE_PENALTY_RETURN_EPERM (constant)
```

Causes system calls to just return an `EPERM` error
...
### PLEDGE_STDERR_LOGGING

```lua
unix.PLEDGE_STDERR_LOGGING (constant)
```

Enables friendly error message logging letting you
...
- `template` (string): template path ending in XXXXXX
