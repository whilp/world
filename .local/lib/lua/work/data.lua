...
-- Default Work{} callback
local function default_work_callback(item)
  ...
  if M.items[item.id] then
    local existing = M.items[item.id]
...
M.load_file = function(path, kinds)
  ...
  if not env.Work then
    env.Work = default_work_callback
  end
