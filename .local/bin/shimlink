...
local function get_config_path()
  return path_join(get_config_dir(), "shimlink.json")
end

local function get_platform_config_path()
  return path_join(get_config_dir(), get_platform() .. ".json")
end
...
local function load_config()
  ...
  local general_config_path = get_config_path()
  ...
  if not is_directory(config_dir) then
    ...
    stderr_write("shimlink: create ~/.config/shimlink/ with shimlink.json and <platform>.json files")
    os.exit(1)
  end
  ...
  if not file_exists(platform_config_path) then
    ...
    stderr_write("shimlink: create ~/.config/shimlink/" .. get_platform() .. ".json with binary configurations")
    os.exit(1)
  end
  ...
  if platform_config.binaries then
    if not merged_config.binaries then
      merged_config.binaries = {}
    end
    for k, v in pairs(platform_config.binaries) do
      merged_config.binaries[k] = v
    end
  end

  if platform_config.aliases then
    if not merged_config.aliases then
      merged_config.aliases = {}
    end
    for k, v in pairs(platform_config.aliases) do
      merged_config.aliases[k] = v
    end
  end
...
local function needs_download(binary_name, config)
  ...
  local executable_path = binary_config.executable_path
  local binary_path

  if executable_path then
    binary_path = path_join(bin_dir, executable_path)
  else
  ...
  local sha256_value = binary_config.sha256
  if not sha256_value or sha256_value == "SKIP_CHECKSUM" then
    return false
  end
...
-- Test binary
local function test_binary(binary_path, test_command)
  local cmd = test_command:gsub("{binary}", binary_path)
...
local function download_binary(binary_name, config, force)
  ...
  local expected_sha256 = binary_config.sha256 or "SKIP_CHECKSUM"
  local executable_path = binary_config.executable_path
  ...
  if executable_path then
    ...
    local err
    extracted_binary, err = extract_archive(download_path, extract_temp, executable_path)
  ...
  elseif expected_sha256 and expected_sha256 ~= "SKIP_CHECKSUM" then
    if actual_sha256 ~= expected_sha256 then
  ...
  local test_command = binary_config.test_command
  if test_command then
    local test_binary_path
    if executable_path then
      test_binary_path = path_join(bin_dir, executable_path)
    else
      test_binary_path = path_join(bin_dir, binary_name)
    end

    if test_binary(test_binary_path, test_command) then
      stderr_write("shimlink: " .. binary_name .. " test passed")
    else
      stderr_write("shimlink: " .. binary_name .. " test failed")
    end
  end
